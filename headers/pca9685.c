...
void pca9685_init_50(uint8_t address){
    ...
    /* Calculate frequency prescalar for usage below. PCA9685 contains a 25 MHz
       ...
       without needing a float) */
    // 121 is for 50Hz
    volatile uint8_t prescalar = 121;
...
//                         ((float)(MAX_PULSE - MIN_PULSE) / (1 * MAX_ANGLE));
...
void pca9685_servo_with_value(uint8_t servoNum, uint32_t angle_value) {
   ...
   //                      ((float)(MAX_PULSE - MIN_PULSE) / (2 * MAX_ANGLE));
    ...
    /* Convert pulse duration into a value from 0 to 4096, which will be
       repeated every (1 / frequency) * 1000000 us */
   //  uint16_t period_us = (float)1000000 / globalFrequency;
   uint16_t count = (angle_value + SCALE_ROUNDUP) / SCALING_FACTOR;
...
uint32_t delta_value(enum delta val){
   switch(val){
      case O1:
         return DELTA_ANGLE_1;
      case O_1:
         return DELTA_ANGLE_0_1;
      case O_5:
         return DELTA_ANGLE_0_5;
      case O_01:
         return DELTA_ANGLE_0_01;
      case O_05:
         return DELTA_ANGLE_0_05;
      case O_25:
         return DELTA_ANGLE_0_25;
      default:
         return DELTA_ANGLE_0_01;
   }
}